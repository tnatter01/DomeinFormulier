...
if(isset($_GET['domain']))
{
		...
		foreach($extensions as $extension => $who)
		{
			...
			if(eregi($who[1], $buffer))
			{
				$extzonderpunt = str_replace(".", "", $extension);
					...
					echo '<h4 class="available"><input type="checkbox" id="aanvraagcheck" " value="'.$extzonderpunt.'" ><span> Beschikbaar</span>' . $domain. '<b>' . $extension .'</b> is vrij 
							</h4>';
			}
			else
			{
	...
	echo '<button id="aanvraagbutton" onclick="aanvraag()">Aanvragen</button>';
}
?>
